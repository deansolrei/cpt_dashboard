/**
 * Solrei SLAVE – Rate Sync from MASTER (v8 — 26-plan column layout)
 *
 * Changes from v7:
 *  - 26 plan groups (added Ambetter WA, BCBS MN Medicaid, BCBS MN Medicaid Advantage,
 *    split Anthem CO into HMO/PPO, added Regence OR, renamed Wellmark → Wellmark Iowa)
 *  - All BCBS column positions shifted right to accommodate new plans
 *  - Master data read extended to 106 columns (covers all 26 plan groups)
 *
 * HOW TO USE:
 ...
 *
 * NOTE: Update MASTER_SHEET_ID below to the new v3 MASTER spreadsheet ID
 * (found in the URL after running buildNewMasterSheet).
 */

// ── !! UPDATE THIS to the new v3 MASTER spreadsheet ID !! ─────
...
// ── State block start rows in MASTER (1-indexed, alphabetical) ─
// KS / ME / VT exist in MASTER but are excluded from tool export
var STATE_TO_ROW = {
...
// ── Plan groups — matches v3 MASTER column layout exactly ─────
...
var PLAN_GROUPS = [
  ...
  { plan: 'Ambetter (Washington)',                             col: 22  },
  { plan: 'BCBS - Florida Blue',                               col: 26  },
  { plan: 'BCBS - Florida Blue Medicare Advantage',            col: 30  },
  { plan: 'BCBS - of Arizona',                                 col: 34  },
  { plan: 'BCBS - of Massachusetts',                           col: 38  },
  { plan: 'BCBS - of Minnesota',                               col: 42  },
  { plan: 'BCBS - Minnesota Medicaid',                         col: 46  },
  { plan: 'BCBS - Minnesota Medicaid Advantage',               col: 50  },
  { plan: 'BCBS - Anthem (Colorado HMO)',                      col: 54  },
  { plan: 'BCBS - Anthem (Colorado PPO)',                      col: 58  },
...
function syncRatesFromMaster() {
  if (MASTER_SHEET_ID === 'PASTE_V3_MASTER_SHEET_ID_HERE') {
    SpreadsheetApp.getUi().alert(
      'Setup required:\n\n' +
      'Open populate_slave_v8.gs and replace\n' +
      'PASTE_V3_MASTER_SHEET_ID_HERE\n' +
      'with the ID from the new v3 MASTER sheet URL.\n\n' +
      'Find the ID in the sheet URL after running buildNewMasterSheet.'
    );
    return;
  }
  ...
  for (var si = 0; si < states.length; si++) {
    ...
    for (var pi = 0; pi < PLAN_GROUPS.length; pi++) {
      ...
      for (var ii = 0; ii < INTERMEDIARIES.length; ii++) {
        ...
        // Effective date from date row (masterData[stateRow] = 0-indexed date row)
