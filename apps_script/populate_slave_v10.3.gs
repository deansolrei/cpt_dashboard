...
// ══════════════════════════════════════════════════════════════
//  MEDICARE RATES SYNC (v10.3 addition)
//  Reads col AL (statewide) from MASTER_DATA tab
//  Writes to medicare_rates tab in SLAVE sheet
//  Download that tab as CSV and upload to dashboard
//  Run annually when Medicare rates update
// ══════════════════════════════════════════════════════════════

function syncMedicareRates() {
  var masterSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(MASTER_TAB);
  if (!masterSheet) {
    SpreadsheetApp.getUi().alert('MASTER_DATA tab not found.');
    return;
  }

  // Read 220 rows × 42 cols (covers col AN = index 39)
  var data = masterSheet.getRange(1, 1, 220, 42).getValues();

  var MEDICARE_COL        = 37;  // col AL (1-indexed) = index 36 (0-indexed)
  var MEDICARE_COL_FTL    = 38;  // col AM = Fort Lauderdale (FL only)
  var MEDICARE_COL_MIA    = 39;  // col AN = Miami (FL only)

  var CPT_OFFSETS = { '99214': 1, '99215': 2, '90833': 3, '90836': 4, '90838': 5 };

  var slaveSS = SpreadsheetApp.getActiveSpreadsheet();
  var sheet   = slaveSS.getSheetByName('medicare_rates');
  if (!sheet) { sheet = slaveSS.insertSheet('medicare_rates'); }
  sheet.clearContents();
  ...
  var outputRows = [];
  var states     = Object.keys(STATE_TO_ROW);
  var cptCodes   = Object.keys(CPT_OFFSETS);

  for (var si = 0; si < states.length; si++) {
    var state     = states[si];
    var stateRow  = STATE_TO_ROW[state];   // 1-indexed label row
    var dateRowIdx = stateRow;              // 0-indexed date row

    for (var ki = 0; ki < cptCodes.length; ki++) {
      var cpt      = cptCodes[ki];
      var dataIdx  = dateRowIdx + CPT_OFFSETS[cpt];  // 0-indexed

      // Statewide rate (col AL, 0-indexed = 36)
      var val = _parseRate(data[dataIdx][36]);
      if (val !== null) {
        outputRows.push([state, state, cpt, val]);
      }

      // Florida localities
      if (state === 'FL') {
        var ftl = _parseRate(data[dataIdx][37]);
        var mia = _parseRate(data[dataIdx][38]);
        if (ftl !== null) outputRows.push(['FL', 'FL-FTL', cpt, ftl]);
        if (mia !== null) outputRows.push(['FL', 'FL-MIA', cpt, mia]);
      }
    }
  }

  if (outputRows.length > 0) {
    sheet.getRange(2, 1, outputRows.length, 4).setValues(outputRows);
    sheet.getRange(2, 4, outputRows.length, 1).setNumberFormat('$#,##0.00');
  }

  SpreadsheetApp.getActiveSpreadsheet().toast(
    outputRows.length + ' Medicare rates written to medicare_rates tab\n' +
    'Download as CSV → upload to dashboard via Rate Template → Medicare Import',
    'Medicare Sync Complete', 10
  );

  Logger.log('Medicare rates synced: ' + outputRows.length + ' rows');
}
