-- 17_fix_channel_direct_rate.sql
-- Fix: v_channel_comparison was using pe.entity_type = 'NPI2' (group entity)
-- for direct rates, while the Rate Comparison table prefers NPI1 (individual
-- provider — Jodene Jensen). This caused Florida Blue (and any other payer
-- with both NPI1 and NPI2 contracts) to show different rates in each section.
--
-- Fix: direct_rates CTE now uses DISTINCT ON ordered so NPI1 rows win;
-- falls back to NPI2 if no NPI1 contract exists for that payer.
--
-- Run: psql solrei_cpt -f 17_fix_channel_direct_rate.sql

SELECT '=== Rebuilding v_channel_comparison with NPI1-preferred direct rates ===' AS info;
...
-- DISTINCT ON (payer, CPT) → one row per pair, NPI1 wins on ties.
direct_rates AS (
    ...
    ORDER BY
        p.payer_name,
        fsl.cpt_code,
        CASE pe.entity_type WHEN 'NPI1' THEN 0 ELSE 1 END,  -- NPI1 first
        fsl.allowed_amount DESC                               -- highest rate if tied
),
...
-- Payer name mapping: one row per intermediary_payer_name
name_resolved AS (
...
-- Expected: total_rows = unique_payer_cpt_pairs, duplicate_pairs = 0

-- Spot-check Florida Blue direct rate — should now match Rate Comparison section
